...
void push_button(const gchar *name)
{
	...
	button = GTK_TOGGLE_TOOL_BUTTON(lookup_widget(appwindow, name));
...
{
	GtkStatusbar *b = GTK_STATUSBAR(data);
...
void statusbar_set(const gchar *name, const gchar *message)
{
	struct statusbar *s;
	GtkStatusbar *b;

	if ((b = GTK_STATUSBAR(lookup_widget(appwindow, name))) == NULL)
		return;
	...
	/* main status bar is a special case, set up a clearing timeout */
	if (!strcmp(name, "mainstatusbar")) {
		if (main_timeout_id)
			g_source_remove(main_timeout_id);

		main_timeout_id = g_timeout_add(10000, 
						statusbar_timeout_callback, 
						(gpointer) b);
	}
}
...
//		gchar *dir;
		...
		w = create_fileselection();
		f = GTK_FILE_SELECTION(w);

		gtk_window_set_title(GTK_WINDOW(w), _("Select File to Transmit"));
...
//		g_free(file);
	...
	if (g_file_get_contents(file, &contents, NULL, &err) == FALSE) {
		if (err != NULL)
...
int main(int argc, char *argv[])
{
	...
	/* select correct gui option for the mode */
	on_mode_activate(trx_get_mode());
	...
	if (cwirc_extension_mode) {
		...
		case MODE_QPSK31:
			...
			break;
		case MODE_BPSK63:
			w = find_menu_item("_Mode/PSK63 (BPSK)");
			break;
		case MODE_QPSK63:
			w = find_menu_item("_Mode/PSK63 (QPSK)");
			break;
		case MODE_MT63:
		...
		case MODE_CW:
			...
			break;
		case MODE_DOMINOEX4:
			w = find_menu_item("_Mode/DominoEX 4");
			break;
		case MODE_DOMINOEX5:
			w = find_menu_item("_Mode/DominoEX 5");
			break;
		case MODE_DOMINOEX8:
			w = find_menu_item("_Mode/DominoEX 8");
			break;
		case MODE_DOMINOEX11:
			w = find_menu_item("_Mode/DominoEX 11");
			break;
		case MODE_DOMINOEX16:
			w = find_menu_item("_Mode/DominoEX 16");
			break;
		case MODE_DOMINOEX22:
			w = find_menu_item("_Mode/DominoEX 22");
			break;
		}
	...
	waterfall_set_frequency(waterfall, (trx_get_freq() > 0.0) ? trx_get_freq() : 1000.0);
	...
	xsize = conf_get_int("misc/xsize");
	ysize = conf_get_int("misc/ysize");
	gtk_window_resize(GTK_WINDOW(appwindow), xsize, ysize);
	...
	gtk_window_get_size(GTK_WINDOW(appwindow), &xsize, &ysize);
	conf_set_int("misc/xsize", xsize);
	conf_set_int("misc/ysize", ysize);
