...
static gchar *mkkey(const gchar *key)
{
	...
	g_snprintf(buf, sizeof(buf), "/apps/%s/%s", PACKAGE_NAME, key);
	buf[sizeof(buf) - 1] = 0;
...
static struct conf_default conf_defs[] = {
	...
	{ "server/host",     D_STRING, { .s = "127.0.0.1" } },
	{ "server/port",     D_STRING, { .s = "9999" } },
	{ "server/password", D_STRING, { .s = "" } },
	...
	{ "misc/xsize",      D_INT,    { .i = 800    } },
	{ "misc/ysize",      D_INT,    { .i = 600    } },
...
gchar *conf_get_string(const gchar *key)
{
	...
	if (s == NULL)
		return conf_get_string_default(key);

	return g_strdup(s);
}

gchar *conf_get_string_default(const gchar *key)
{
...
gboolean conf_get_bool(const gchar *key)
{
	...
	if (!value || value->type != GCONF_VALUE_BOOL)
		return conf_get_bool_default(key);

	return gconf_value_get_bool(value);
}

gboolean conf_get_bool_default(const gchar *key)
{
	struct conf_default *defs = conf_defs;
...
gint conf_get_int(const gchar *key)
{
	...
	if (!value || value->type != GCONF_VALUE_INT)
		return conf_get_int_default(key);

	return gconf_value_get_int(value);
}

gint conf_get_int_default(const gchar *key)
{
	struct conf_default *defs = conf_defs;
...
gdouble conf_get_float(const gchar *key)
{
	...
	if (!value || value->type != GCONF_VALUE_FLOAT)
		return conf_get_float_default(key);

	return gconf_value_get_float(value);
}

gdouble conf_get_float_default(const gchar *key)
{
	struct conf_default *defs = conf_defs;
